...
// この設定の狙い: 6車種で最速。トラクションと安定性を攻めに使う本命。重い分コーナーは早めに減速して膨らみを防ぐ。

int TOP = 255;      // 直線の最高 PWM
int CRUISE = 226;   // 中速域の PWM
int SLOW = 148;     // コーナーの PWM
int TCAP = 208;     // 旋回中の上限 PWM (低いほどアクセルを抜く)
int D_OPEN = 645;   // 前方[mm] これ以上=直線とみなし全開
int D_MID = 410;    // 前方[mm] これ以上=中速、未満=減速
int D_TURN = 386;   // 前方[mm] これ未満=広い方へ曲げる
int D_SIDE = 178;   // 側方[mm] これ未満=壁から離れる
